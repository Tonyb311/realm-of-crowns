...
export const townMetricsRelations = relations(townMetrics, ({one}) => ({
	town: one(towns, {
...
export const blackMarketListingsRelations = relations(blackMarketListings, ({one}) => ({
	town: one(towns, {
		fields: [blackMarketListings.townId],
		references: [towns.id],
	}),
	seller: one(characters, {
		fields: [blackMarketListings.sellerId],
		references: [characters.id],
	}),
	item: one(items, {
		fields: [blackMarketListings.itemId],
		references: [items.id],
	}),
	itemTemplate: one(itemTemplates, {
		fields: [blackMarketListings.itemTemplateId],
		references: [itemTemplates.id],
	}),
}));

export const racialReputationsRelations = relations(racialReputations, ({one}) => ({
	character: one(characters, {
		fields: [racialReputations.characterId],
		references: [characters.id],
	}),
}));
...
export const disputesRelations = relations(disputes, ({one}) => ({
	town: one(towns, {
		fields: [disputes.townId],
		references: [towns.id],
	}),
	filer: one(characters, {
		fields: [disputes.filerId],
		references: [characters.id],
		relationName: 'disputeFiler',
	}),
	target: one(characters, {
		fields: [disputes.targetId],
		references: [characters.id],
		relationName: 'disputeTarget',
	}),
	arbiter: one(characters, {
		fields: [disputes.arbiterId],
		references: [characters.id],
		relationName: 'disputeArbiter',
	}),
}));

export const referendumsRelations = relations(referendums, ({one, many}) => ({
	town: one(towns, {
		fields: [referendums.townId],
		references: [towns.id],
	}),
	proposedBy: one(characters, {
		fields: [referendums.proposedById],
		references: [characters.id],
	}),
	votes: many(referendumVotes),
}));

export const referendumVotesRelations = relations(referendumVotes, ({one}) => ({
	referendum: one(referendums, {
		fields: [referendumVotes.referendumId],
		references: [referendums.id],
	}),
	character: one(characters, {
		fields: [referendumVotes.characterId],
		references: [characters.id],
	}),
}));

export const townHistoryLogRelations = relations(townHistoryLog, ({one}) => ({
	town: one(towns, {
		fields: [townHistoryLog.townId],
		references: [towns.id],
	}),
	involvedCharacter: one(characters, {
		fields: [townHistoryLog.involvedCharacterId],
		references: [characters.id],
	}),
}));

export const townUpgradesRelations = relations(townUpgrades, ({one}) => ({
	town: one(towns, {
		fields: [townUpgrades.townId],
		references: [towns.id],
	}),
...
export const townProclamationsRelations = relations(townProclamations, ({one}) => ({
	town: one(towns, {
		fields: [townProclamations.townId],
		references: [towns.id],
	}),
	author: one(characters, {
		fields: [townProclamations.authorId],
		references: [characters.id],
	}),
}));
