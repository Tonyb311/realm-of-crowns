...
import type { NpcRole } from '@shared/enums';

// Quest data imported from shared (already compiled)
import { TUTORIAL_QUESTS } from '@shared/data/quests';

export async function seedQuests(db: any): Promise<void> {
  console.log('--- Seeding Quests (clean slate) ---');

  // 1. Delete ALL existing quest_progress records
  const deletedProgress = await db.delete(schema.questProgress);
  console.log(`  Deleted ${deletedProgress.rowCount ?? 0} quest_progress records`);

  // 2. Delete ALL existing NPC records with role QUEST_GIVER
  const deletedNpcs = await db.delete(schema.npcs).where(eq(schema.npcs.role, 'QUEST_GIVER' as NpcRole));
  console.log(`  Deleted ${deletedNpcs.rowCount ?? 0} QUEST_GIVER NPCs`);

  // 3. Delete ALL existing quest records (clear prerequisite links first)
  await db.update(schema.quests).set({ prerequisiteQuestId: null });
  const deletedQuests = await db.delete(schema.quests);
  console.log(`  Deleted ${deletedQuests.rowCount ?? 0} quests`);
  ...
  for (const questDef of TUTORIAL_QUESTS) {
    const [created] = await db.insert(schema.quests).values({
  ...
  for (const questDef of TUTORIAL_QUESTS) {
    ...
    if (dbId && prereqDbId) {
      await db.update(schema.quests).set({ prerequisiteQuestId: prereqDbId }).where(eq(schema.quests.id, dbId));
      linkedCount++;
    }
  }
  ...
  let town = await db.query.towns.findFirst({ where: eq(schema.towns.name, 'Kingshold') });
  if (!town) {
    town = await db.query.towns.findFirst();
  }
