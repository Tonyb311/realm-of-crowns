...
import { seedTownMetrics } from './town-metrics';
...
async function runSeed(name, fn) {
    try {
        ...
        console.error(`⚠️  Seed "${name}" failed (non-fatal):`, e.message);
        return false;
    }
}
async function main() {
    ...
    if (!await runSeed('baseValuePropagation', () => seedBaseValuePropagation(db)))
        failed++;
    console.log('');
    if (failed > 0) {
