import { pgTable, varchar, timestamp, text, integer, uniqueIndex, index, foreignKey, doublePrecision, jsonb, date, boolean, type AnyPgColumn, pgEnum } from "drizzle-orm/pg-core"
...
export const equipSlot = pgEnum("EquipSlot", ['HEAD', 'CHEST', 'HANDS', 'LEGS', 'FEET', 'MAIN_HAND', 'OFF_HAND', 'RING_1', 'RING_2', 'NECK', 'BACK', 'TOOL'])
...
export const characterEquipment = pgTable("character_equipment", {
	...
	foreignKey({
			...
			name: "character_equipment_item_id_fkey"
		}).onUpdate("cascade").onDelete("cascade"),
]);
...
export const kingdoms = pgTable("kingdoms", {
	...
	foreignKey({
			columns: [table.rulerId],
			foreignColumns: [characters.id],
			name: "kingdoms_ruler_id_fkey"
...
export const towns = pgTable("towns", {
	...
	foreignKey({
			columns: [table.mayorId],
			foreignColumns: [characters.id],
			name: "towns_mayor_id_fkey"
		}).onUpdate("cascade").onDelete("set null"),
	foreignKey({
...
export const characters = pgTable("characters", {
	...
	preferredFoodId: text("preferred_food_id"),
	...
	foreignKey({
			columns: [table.preferredFoodId],
			foreignColumns: [items.id],
			name: "characters_preferred_food_id_fkey"
		}).onUpdate("cascade").onDelete("set null"),
	foreignKey({
