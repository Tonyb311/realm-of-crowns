...
router.post('/start', authGuard, validate(startPveSchema), async (_req: AuthenticatedRequest, res: Response) => {
  return res.status(400).json({
    error: 'PvE combat now occurs as road encounters during travel. Head to the Travel page to begin your journey — monsters roam the roads between towns.',
  });
});
...
function formatCombatResponse(state: CombatState) {
  const log = formatCombatLog(state);
