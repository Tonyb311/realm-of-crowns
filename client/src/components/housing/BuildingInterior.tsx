    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-realm-border">
          <div>
            <h3 className="font-display text-lg text-realm-gold-400">{building.name}</h3>
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-0.5">
              {Array.from({ length: 5 }).map((_, i) => (
                <Star
                  key={i}
                  className={`w-4 h-4 ${i < building.level ? 'text-realm-gold-400 fill-realm-gold-400' : 'text-realm-border'}`}
                />
              ))}
            </div>
          ...
          {building.hasStorage && storage && (
            <div>
              <div className="flex items-center justify-between mb-3">
                <h4 className="font-display text-sm text-realm-text-primary flex items-center gap-2">
                  <Box className="w-4 h-4 text-realm-gold-400" />
                  Storage
                </h4>
                <span className="text-xs text-realm-text-muted">
              ...
              {storage.items.length === 0 ? (
                ...
                <div className="space-y-1.5 mb-3">
                  {storage.items.map((item) => (
                    ...
                    >
                      <Package className="w-3.5 h-3.5 text-realm-text-muted flex-shrink-0" />
                      <span className="text-sm text-realm-text-primary flex-1">{item.itemName}</span>
                      <span className="text-xs text-realm-text-muted font-display">x{item.quantity}</span>
                    </div>
                  ))}
                </div>
              )}
                ...
                <div className="bg-realm-bg-900 border border-realm-border rounded p-3 mb-3">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className="px-2 py-1.5 text-realm-text-muted hover:text-realm-text-primary text-xs"
                    >
                      Cancel
                    </button>
                  </div>
              ...
              <div className="bg-realm-bg-900 border border-realm-border rounded p-3">
                ...
                {inventory.length === 0 ? (
                  <p className="text-realm-text-muted text-xs">Your inventory is empty.</p>
                ) : (
                  <div className="flex gap-2">
                    <select
                      value={depositItemId}
          ...
          {building.hasStorage && (
            <div>
              <h4 className="font-display text-sm text-realm-text-primary mb-2">Rooms</h4>
              ...
              <p className="text-[10px] text-realm-text-muted/60 mt-1 text-center">
