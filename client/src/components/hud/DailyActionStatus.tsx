...
export function DailyActionStatus() {
  ...
  if (actionUsed) {
    return (
      <Tooltip content={`Daily action used: ${actionType ?? 'Unknown'}. Resets at next tick.`} position="bottom">
        <div className="flex items-center gap-1.5 px-2 py-1 rounded-sm bg-realm-bg-600/40 border border-realm-border">
  ...
  return (
    <Tooltip content="You have 1 daily action available. Gather, travel, or craft!" position="bottom">
      <div className="flex items-center gap-1.5 px-2 py-1 rounded-sm bg-realm-gold-400/10 border border-realm-gold-400/30 animate-pulse-subtle">
