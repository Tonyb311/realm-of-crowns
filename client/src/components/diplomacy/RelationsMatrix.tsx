...
const STATUS_COLORS: Record<RelationStatus, { bg: string; border: string; text: string; label: string }> = {
  ...
  DISTRUSTFUL: { bg: 'bg-amber-500',   border: 'border-amber-400',   text: 'text-amber-400',   label: 'Distrustful' },
  HOSTILE:     { bg: 'bg-orange-500',   border: 'border-orange-400',  text: 'text-orange-400',  label: 'Hostile' },
  BLOOD_FEUD:  { bg: 'bg-red-700',     border: 'border-red-500',     text: 'text-red-400',     label: 'Blood Feud' },
};
...
export default function RelationsMatrix() {
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center py-20">
        ...
        <span className="ml-2 text-parchment-300 font-display">Loading relations...</span>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3 px-1">
        {Object.entries(STATUS_COLORS).map(([status, c]) => (
          <div key={status} className="flex items-center gap-1.5">
            ...
            <span className="text-parchment-300 text-xs">{c.label}</span>
          </div>
        ))}
      </div>

      {/* Matrix */}
      <div className="overflow-auto max-h-[70vh] border border-dark-50 rounded-lg">
        <table className="border-collapse">
          <thead>
            <tr>
              <th className="sticky left-0 top-0 z-20 bg-dark-500 p-0" />
              {RACES.map((race, ci) => (
                <th
                  key={race}
                  className={`sticky top-0 z-10 bg-dark-500 px-1 py-1.5 text-[10px] font-display text-parchment-500 ${
                    hoveredCell?.col === ci ? 'text-primary-400' : ''
                  }`}
          ...
          <tbody>
            {RACES.map((rowRace, ri) => (
              <tr key={rowRace}>
                <td
                  className={`sticky left-0 z-10 bg-dark-500 px-2 py-1 text-[10px] font-display text-parchment-500 whitespace-nowrap ${
                    hoveredCell?.row === ri ? 'text-primary-400' : ''
                  }`}
                >
                  {RACE_ABBR[rowRace]}
                </td>
                    ...
                    >
                      <div
                        className={`w-5 h-5 rounded-sm ${colors.bg} ${
                          isHovered ? 'ring-1 ring-white/50 scale-125' : ''
      ...
      {/* Hover tooltip */}
      {hoveredCell && (
        <div className="text-xs text-parchment-300 px-1">
          <span className="text-primary-400 font-display">{formatRaceName(RACES[hoveredCell.row])}</span>
          {' / '}
          <span className="text-primary-400 font-display">{formatRaceName(RACES[hoveredCell.col])}</span>
          {' — '}
...
function RelationDetailDrawer({
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: 20 }}
      className="bg-dark-400 border border-dark-50 rounded-lg p-4 mt-2"
    >
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <h3 className="font-display text-primary-400 text-sm">
            {formatRaceName(race1)} &mdash; {formatRaceName(race2)}
          </h3>
          <span className={`px-2 py-0.5 rounded-full text-[10px] border ${colors.text} ${colors.border} bg-dark-500`}>
            {colors.label}
          </span>
        </div>
      ...
      {detail && (
        <div className="text-xs text-parchment-300 mb-3">
          Relation Score: <span className="text-parchment-200 font-display">{detail.score}</span>
        </div>
      )}

      <div className="flex items-center gap-1.5 mb-2">
        <History className="w-3.5 h-3.5 text-parchment-500" />
        <span className="text-parchment-500 text-xs font-display">Diplomatic History</span>
      </div>

      {isLoading ? (
        <div className="flex items-center gap-2 py-4">
          <Loader2 className="w-4 h-4 text-primary-400 animate-spin" />
          <span className="text-parchment-500 text-xs">Loading history...</span>
        </div>
      ) : !history || history.length === 0 ? (
        <p className="text-parchment-500 text-xs py-2">No diplomatic events recorded between these races.</p>
      ) : (
        <div className="space-y-1.5 max-h-48 overflow-y-auto">
          {history.map(evt => (
            <div key={evt.id} className="flex items-start gap-2 text-xs">
              <span className="text-parchment-500 whitespace-nowrap shrink-0">
